...
#if(file.exists(paste("chr",chr,"_fix_p_permutation_residQN_sub",i,".txt",sep=""))){
# dat=read.table(paste("chr",chr,"_fix_p_permutation_residQN_sub",i,".txt",sep=""),header=T,as.is=T)
#for(nc in 1:ncol(dat)){
#gene=gene.names[nc]
#flag=which(gene_pos$gene==gene)
#start=max(0,flag-50)
...
#write.table(allperm,paste("chr",chr,"_perm_residQN_allp.txt",sep=""),quote=F,row.names=F,col.names=F)
