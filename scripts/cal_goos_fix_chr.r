...
output_dir=args[9]
sample_dir=args[10]
pos_gene=read.table(gene_pos_dir,header=T,as.is=T,sep="\t")
allmap=read.table(paste(genotype_dir,"chr",chr,"_QCed.bim",sep=""))
...
goos=NULL
colgenes=NULL
sv=read.table(sva_dir,header=T,as.is=T)
pc=read.table(genpc_dir,as.is=T)
if(!is.na(pc)){
	pc=as.matrix(pc[,1:10])
...
for(flag_i in chr_flag){
	...
	if(!( length(exp_gflag)==0)){
		cat(flag_i)
		cat("\n")
		tss1=max(0,pos_gene$start[flag_i]-100000)
		tss2=pos_gene$start[flag_i]+100000
		...
		if(length(cis_flag)>0){
			...
			system(paste(plink_dir,"plink --bfile ",genotype_dir,"chr",chr,"_QCed --keep ",sample_dir," --extract chr",chr,"_sub",sub,".txt --recode12 --range --out chr",chr,"_sub",sub,sep=""))
...
write.table(goos,paste(output_dir),col.names=T,row.names=F,sep="\t")
