...
gname=read.table("gene_pos.txt",header=T,sep="\t")
gmap=read.table("../../avg_mappability_nondup.txt",header=T,row.names=1)
genelist=read.table("gene_pos.txt",header=T,sep="\t")
num_reg=NULL
reg_all=NULL
mgenes=NULL
mcor=NULL
pseudo=read.table("../../pseudogenes.txt",as.is=T)
gene1=NULL
gene2=NULL
transp=NULL
for(sub in 1:10){
r2=read.table(paste("pearsonR_chr",chr,"_sub",sub,".txt",sep=""),header=T,sep="\t")
cor2=read.table(paste("pearsonR_chr",chr,"_sub",sub,".txt",sep=""),header=T,sep="\t")
if(file.exists(paste("cor_chr",chr,"_sub",sub,".txt",sep=""))){
dat=read.table(paste("cor_chr",chr,"_sub",sub,".txt",sep=""),header=T,check.names=F)
...
write.table(out,paste("chr",chr,"_sig_fdr10_genepairs.txt",sep=""),row.names=F,sep="\t",quote=F)
